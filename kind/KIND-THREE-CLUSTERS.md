...
- **ClusterMesh (spoke):** Hub/spoke cần **cùng** cấu hình encryption với management (`cilium/cilium-values.yaml`). IP hub: `bash scripts/kind-clustermesh-peer-ip.sh` hoặc gộp bước TLS + IP: `./scripts/kind-clustermesh-sync-spoke-from-hub.sh`, rồi commit/push và sync Argo `cilium-{dev,staging,prod}`. `cilium clustermesh connect` **không** dùng được khi spoke `useAPIServer: false`.
