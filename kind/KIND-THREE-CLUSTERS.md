...
- ClusterMesh: `bash scripts/kind-clustermesh-status.sh` (hoặc `cilium clustermesh status --context kind-management`). Spoke (`useAPIServer: false`): sau recreate Kind chạy `./scripts/kind-clustermesh-sync-spoke-from-hub.sh`, commit/push `cilium/clustermesh-management-peer.yaml`, sync `cilium-{dev,staging,prod}`; encryption spoke phải khớp hub (xem mục **3** / `cilium-values.yaml`).
...
- **ClusterMesh (spoke):** Hub/spoke cần **cùng** cấu hình encryption với management (`cilium/cilium-values.yaml`). **Hub** phải khai báo tên cluster workload trong `cilium-values-management.yaml` → `clustermesh.config.clusters` (dev/staging/prod); nếu để `{}` thì Secret `cilium-clustermesh` trên management **rỗng** và `cilium clustermesh status` **luôn** báo *No cluster connected* (CLI đọc secret để biết peer). IP spoke→hub: `bash scripts/kind-clustermesh-peer-ip.sh` hoặc `./scripts/kind-clustermesh-sync-spoke-from-hub.sh`; sync Argo `cilium-management` rồi `cilium-{dev,staging,prod}`. `cilium clustermesh connect` **không** dùng được khi spoke `useAPIServer: false`.
